...
public class AppointmentServiceImpl implements AppointmentService {
    ...
    public PageResponse<AppointmentResponse> getAppointments(UUID userId, Status status,
                                                             Pageable pageable) {
        log.info("Getting appointments: userId={}, status={}", userId, status);
        
        Page<Appointment> appointments = appointmentRepository
                .findAppointmentsByUserIdAndStatus(userId, status, pageable);
        
        return pageMapper.toPageResponse(appointments, factory::toResponse);
    }

    @Override
    public List<AppointmentInternalResponse> getAffectedFullDay(UUID doctorId, LocalDate date) {
        log.info("Getting affected appointments for full day: doctorId={}, date={}", doctorId, date);
        
        List<Appointment> appointments = appointmentRepository
                .findByDoctorIdAndDateAndActiveStatus(doctorId, date);
        
        return appointments.stream()
                .map(factory::toInternalResponse)
                .toList();
    }
    ...
    public List<AppointmentInternalResponse> getAffectedByTimeRange(
        ...
        log.info("Getting affected appointments by time range: doctorId={}, date={}, time={}-{}",
                doctorId, date, startTime, endTime);
        
        List<Appointment> appointments = appointmentRepository
                .findOverlappingAppointmentsByTimeRange(doctorId, date, startTime, endTime);
    ...
    private String generateUniquePublicCode() {
        ...
        while (attempts < MAX_RETRY_ATTEMPTS) {
            ...
            attempts++;
            log.info("Mã tạo public code: {}", publicCode);
        }
